...
class Splash {
    ...
    async maintenanceCheck() {
        config.GetConfig().then(res => {
            ...
            console.error(e);
            return this.shutdown("No se detectó conexión a Internet. Inténtelo de nuevo más tarde.");
        })
    }
    ...
    shutdown(text) {
        this.setStatus(`${text}<br>Detenerse en 5s`);
        let i = 4;
        setInterval(() => {
            this.setStatus(`${text}<br>Detenerse en ${i--}s`);
